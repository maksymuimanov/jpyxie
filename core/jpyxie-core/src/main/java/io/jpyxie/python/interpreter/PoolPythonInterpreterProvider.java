...
public class PoolPythonInterpreterProvider<I extends AutoCloseable> implements PythonReleasableInterpreterProvider<I> {
    ...
    public PoolPythonInterpreterProvider(PythonInterpreterFactory<I> interpreterFactory,
                                         int maxPoolSize,
                                         Duration timeout) {
        this(interpreterFactory, new ArrayBlockingQueue<>(maxPoolSize), maxPoolSize, timeout);
    }

    public PoolPythonInterpreterProvider(PythonInterpreterFactory<I> interpreterFactory,
                                         BlockingQueue<I> pool,
                                         int maxPoolSize,
                                         Duration timeout) {
    ...
    public I acquire(long timeout, TimeUnit unit) {
        log.debug("Acquiring interpreter from pool [available: {}, pool size: {}]", this.pool.size(), this.maxPoolSize);
        if (this.closed.get()) {
            throw PoolPythonInterpreterProviderException.closed();
        }
        try {
            if (this.pool.isEmpty() && this.filled.compareAndSet(false, true)) {
            ...
            if (polled == null) {
               throw PoolPythonInterpreterProviderException.poolStarvation();
            }
            log.debug("Interpreter acquired from pool successfully");
            return polled;
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw PoolPythonInterpreterProviderException.interrupted(e);
        } catch (Exception e) {
            throw PoolPythonInterpreterProviderException.failedToAcquireInterpreter(e);
        }
    }
        ...
        try {
            synchronized (this.pool) {
                ...
                for (int i = 0; i < this.maxPoolSize; i++) {
                    ...
                    if (!offered) {
                        throw PoolPythonInterpreterProviderException.failedToCreateInterpreter();
                    }
                }
                log.debug("Pool filled successfully [available: {}]", this.pool.size());
            }
        } catch (Exception e) {
            throw PoolPythonInterpreterProviderException.failedToFill(e);
        }
    }
    ...
    public void release(@Nullable I interpreter) {
        try {
            ...
            if (this.closed.get()) {
                this.closeInterpreterOnPoolClosed(interpreter);
            } else {
                boolean offered = this.pool.offer(interpreter);
                if (!offered) {
                    throw PoolPythonInterpreterProviderException.failedToReturnInterpreter();
                }
                log.debug("Interpreter returned to pool [available: {}]", this.pool.size());
            }
        } catch (Exception e) {
            throw PoolPythonInterpreterProviderException.failedToReleaseInterpreter(e);
        }
    }

    private void closeInterpreterOnPoolClosed(I interpreter) {
        try {
            log.debug("Pool is closed, closing released interpreter");
            interpreter.close();
        } catch (Exception e) {
            throw PoolPythonInterpreterProviderException.failedToCloseInterpreter(e);
        }
    }
    ...
    public void close() throws Exception {
        try {
            ...
            throw PoolPythonInterpreterProviderException.failedToClose(e);
        }
    }
}